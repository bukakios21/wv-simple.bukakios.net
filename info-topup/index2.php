...
if (isset($_GET['act'])) {
 ...
 // TODO: redis caching removed - $rediw->setex("nomor-$id",3600*2, $nomor);
...
if ($metode_id == 42 || $metode_id == 44 || $metode_id == 46 || $metode_id == 47) {
    $adm = 0.25;
    if ($metode_id == 42) {
        $adm = 0.32;
    }

    $total_bayar_pulsa = ceil(($nominal_topup+($nominal_topup * $adm))/1000) *1000;
    $total_transfer_pulsa_rp = $app->idr($total_bayar_pulsa);
    $total_transfer_pulsa = $total_bayar_pulsa;
}
?>
<!doctype html>
<html lang="en">
                ...
                <div class="text-center mx-5" style="padding:0px !important">
                    ...
                    ?>
                        ...
                        <table width="150%" style="text-align: left; padding:0px; margin-left:-45px">
                            ...
                            <!-- <tr>
                                <td><span class="font-weight-bold text-left">Admin</span></td>
                                <td><span class="font-weight-light"><?php echo $adm*100; ?>%</span></td>
                            </tr> -->
                            <tr>
                                <td><span class="font-weight-bold">Total</span></td>
                                <td><span class="font-weight-light"><?php echo $app->idr($total_transfer) ?></span></td>
                                <!-- <td><span class="font-weight-light"><?php echo $app->idr($total_bayar_pulsa) ?></span></td> -->
                            </tr>
                        </table>
                    <?php
                    } else {
                    ...
                    <?php
                        ...
                        if ($metode_id == 42 || $metode_id == 44 || $metode_id == 46 || $metode_id == 47) {
                            ...
                            <div class="box-money">
                                ...
                                <!-- <h3 id='nominal_transfer' data-text="Jumlah Transfer Berhasil Di Salin" data-copy="<?= $total_transfer_pulsa; ?>"><?= $total_transfer_pulsa_rp; ?></h3> -->
                            </div>
                            <br />
    ...
    <script>
        $(document).on('click', '#cari', function() {
            ...
            if (hp.length < 9) {
                ...
                // console.log(hp)
                $.ajax({
                    url: 'index2.php?act=cek&nom='+ hp+'&id=<?= $topup_id ?>&nominal=<?= $total_transfer ?>&nomor_rekening=<?php echo $nom_rek ?>',
