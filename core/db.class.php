...
class Database{
	...
	public function fetch_multiple_rows($sql){
		$result = mysqli_query($this->mysqli,$sql);
	...
	function get_token_user($uid){
		$cek = $this->num_rows("select id from users where id='$uid'");
		if($cek!=0){
			$data_token_u1=$this->fetch("select token from users where id='$uid'");
			if($data_token_u1['token']!=''){
				return $data_token_u1['token'];
			}else{
				$data_token_u2=$this->fetch("select token from notification_all where uid='$uid'");
				if($data_token_u2['token']!=''){
					return $data_token_u2['token'];
				}else{
					$data_token_u3=$this->fetch("select token from notification_post where uid='$uid'");
					if($data_token_u3['token']!=''){
						return $data_token_u3['token'];
					}else{
						return "null";
					}
				}
