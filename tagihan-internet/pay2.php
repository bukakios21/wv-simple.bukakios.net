...
if (isset($_REQUEST['msg'], $_REQUEST['csrf'])) {
    ...
    if ($_SESSION['csrf'] === $csrf) {
        if ($msg === "cek") {
            ...
            // Debug log inquiry Internet
            if (isset($app) && method_exists($app, 'simpan_file')) {
                $app->simpan_file("inq_net_" . preg_replace('/[^A-Za-z0-9]/', '', $id_pelanggan) . ".json", (string)$cek_tagihan_http);
            }
            ...
            if (isset($_REQUEST['trx_id'], $_REQUEST['biaya_toko'])) {
                ...
                // Debug log bayar Internet
                if (isset($app) && method_exists($app, 'simpan_file')) {
                    $app->simpan_file("pay_net_" . preg_replace('/[^A-Za-z0-9]/', '', $trx_id) . ".json", (string)$bayar_tagihan_http);
                }
...
<head>
  ...
  <title><?= htmlspecialchars($product_name ?? 'Bayar Tagihan Internet') ?></title>
...
<body class="min-h-screen bg-slate-50 font-sans text-slate-950">
  ...
  <script>
    ...
    function showInfo() {
      showModal({
        title: 'Cek Nomor Pelanggan Internet',
        message: 'Nomor pelanggan internet tertera pada struk atau invoice bulanan provider internet. Biasanya berupa angka/kode pelanggan. Jika kesulitan, hubungi provider terkait.',
        variant: 'info',
        btnText: 'Ok'
      });
    }
    ...
    function renderDetail(data) {
      ...
      // Field khusus Internet/Pascabayar (fallback ke '-' jika kosong)
      ...
      // Sembunyikan baris meter_awal / meter_akhir kalau kosong (produk Internet biasanya tidak kirim)
      if (meterAwal === '-') {
