...
if (isset($_REQUEST['msg'], $_REQUEST['csrf'])) {
    ...
    if ($_SESSION['csrf'] === $csrf) {
        if ($msg === "cek") {
            ...
            // Debug log inquiry TV
            if (isset($app) && method_exists($app, 'simpan_file')) {
                $app->simpan_file("inq_tv_" . preg_replace('/[^A-Za-z0-9]/', '', $id_pelanggan) . ".json", (string)$cek_tagihan_http);
            }
            ...
            if (isset($_REQUEST['trx_id'], $_REQUEST['biaya_toko'])) {
                ...
                // Debug log bayar TV
                if (isset($app) && method_exists($app, 'simpan_file')) {
                    $app->simpan_file("pay_tv_" . preg_replace('/[^A-Za-z0-9]/', '', $trx_id) . ".json", (string)$bayar_tagihan_http);
                }
...
<head>
  ...
  <title><?= htmlspecialchars($product_name ?? 'Bayar Tagihan TV') ?></title>
...
<body class="min-h-screen bg-slate-50 font-sans text-slate-950">
    ...
    <section class="px-6 pt-5 pb-4 bg-white">
      <div class="flex items-center gap-3">
        <div class="h-12 w-12 rounded-2xl shadow-card bg-brand/8 flex items-center justify-center overflow-hidden shrink-0">
          <img src="<?= htmlspecialchars($product_logo ?? '') ?>" class="h-full w-full object-contain" alt="TV" onerror="this.style.display='none';this.parentElement.innerHTML='<svg class=\'h-6 w-6 text-brand\' viewBox=\'0 0 24 24\' fill=\'none\' stroke=\'currentColor\' stroke-width=\'1.8\'><rect x=\'2\' y=\'7\' width=\'20\' height=\'15\' rx=\'2\'/><polyline points=\'17 2 12 7 7 2\'/></svg>'"/>
        </div>
        <div>
    ...
    <section class="px-4 pb-4">
      <div class="rounded-[16px] border border-slate-200 bg-white shadow-soft overflow-hidden">
        <div class="px-5 pt-5 pb-4" id="inputSection">
          ...
          <p class="text-[13px] text-mutedText mt-2">Masukkan nomor pelanggan TV <?= htmlspecialchars($product_name) ?></p>
        </div>
  ...
  <script>
    ...
    function showInfo() {
      showModal({
        title: 'Cek Nomor Pelanggan TV',
        message: 'Nomor pelanggan TV tertera pada kartu pelanggan, struk, atau invoice bulanan provider TV. Biasanya berupa angka/kode pelanggan. Jika kesulitan, hubungi provider TV terkait.',
        variant: 'info',
        btnText: 'Ok'
      });
    }
    ...
    function renderDetail(data) {
      ...
      // Field khusus TV/Pascabayar (fallback ke '-' jika kosong)
      ...
      // Sembunyikan baris meter_awal / meter_akhir kalau kosong (produk TV biasanya tidak kirim)
      if (meterAwal === '-') {
