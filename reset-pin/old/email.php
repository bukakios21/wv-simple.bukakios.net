...
<body class="min-h-screen bg-slate-50 font-sans text-slate-950 antialiased pb-8">
  ...
  <main class="px-4 pt-6">
    ...
    <div class="flex flex-col items-center text-center mb-6">
      <div class="w-20 h-20 rounded-2xl bg-red-500/10 flex items-center justify-center mb-4 shadow-soft">
        <svg viewBox="0 0 24 24" class="w-10 h-10 text-red-500" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
          <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/>
          <polyline points="22,6 12,13 2,6"/>
        </svg>
      </div>
      <h1 class="text-[22px] font-extrabold tracking-tight text-slate-900">Verifikasi OTP</h1>
      <p class="text-sm text-slate-500 mt-1">via Email</p>
    </div>
      ...
      <div class="flex items-start gap-3">
        ...
        <div>
          <p class="text-[13px] text-red-800 leading-relaxed">
            Kode OTP telah dikirim ke email kamu. Mohon tunggu beberapa saat dan periksa inbox email.
          </p>
        </div>
      </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-100 p-5 shadow-card">
      <div class="mb-4">
        ...
        <div class="flex justify-center gap-2">
          <input type="text" maxlength="1" inputmode="numeric" pattern="[0-9]" class="otp-input w-12 h-14 text-center text-xl font-bold border border-slate-200 rounded-xl focus:outline-none focus:border-red-500 focus:ring-2 focus:ring-red-500/20 transition">
          <input type="text" maxlength="1" inputmode="numeric" pattern="[0-9]" class="otp-input w-12 h-14 text-center text-xl font-bold border border-slate-200 rounded-xl focus:outline-none focus:border-red-500 focus:ring-2 focus:ring-red-500/20 transition">
          <input type="text" maxlength="1" inputmode="numeric" pattern="[0-9]" class="otp-input w-12 h-14 text-center text-xl font-bold border border-slate-200 rounded-xl focus:outline-none focus:border-red-500 focus:ring-2 focus:ring-red-500/20 transition">
          <input type="text" maxlength="1" inputmode="numeric" pattern="[0-9]" class="otp-input w-12 h-14 text-center text-xl font-bold border border-slate-200 rounded-xl focus:outline-none focus:border-red-500 focus:ring-2 focus:ring-red-500/20 transition">
          <input type="text" maxlength="1" inputmode="numeric" pattern="[0-9]" class="otp-input w-12 h-14 text-center text-xl font-bold border border-slate-200 rounded-xl focus:outline-none focus:border-red-500 focus:ring-2 focus:ring-red-500/20 transition">
      ...
      <!-- Button -->
      <button id="btn-sending" onclick="submitOtp()" class="w-full rounded-xl bg-red-500 py-3.5 text-[15px] font-bold text-white shadow-lg transition hover:bg-red-600 active:scale-[0.98] flex items-center justify-center gap-2">
  ...
  <div id="loading-overlay" style="display:none;position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.5);z-index:9999;align-items:center;justify-content:center;">
    <div style="background:#fff;border-radius:16px;padding:24px;display:flex;flex-direction:column;align-items:center;gap:12px;box-shadow:0 20px 60px rgba(0,0,0,0.15);min-width:200px;">
      <div style="width:44px;height:44px;border:4px solid rgba(239,68,68,0.2);border-top-color:#ef4444;border-radius:50%;animation:spin 0.8s linear infinite;"></div>
