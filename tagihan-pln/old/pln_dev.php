...
$user_jwt = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJleHAiOiIxNzYyNDE4NjQxIiwidG9rZW4iOiI1ODg3ZjIwYThhOTdlMjcxNmZjZTA3OTdkMjE0MDI3MzkyNjczMDA1ZDkwZmFkOWFiNjg3Y2M3MmJhYzdmMTkxIiwidG9rZW5fdHJ4IjoiMGU0MjRlMTAwM2E4OTQ5YmYzOWZhNjcxMTA5NzQ1YjhjNGM5YzM4NDYxZjlmODc2NDJmNjVkMTczNmVkOTI2YSIsInVpZCI6IjM5OTU4In0.idcsB5anENr_ZbLQ89kOWXjXxL69Fj0ht-9LSYzdB_4";
define("JWT", $user_jwt);
$user_id = 39958;
// if ($user_id==39958){
...
$file_me = "pln_dev.php";
$id_produk = '2768'; //data static id produk jarang berubah
...
if (isset($_REQUEST['msg'], $_REQUEST['csrf'])) {
    ...
    if ($_SESSION['csrf'] === $csrf) {
        if ($msg === "cek") {
            ...
            $app->simpan_file("cek.json", $cek_tagihan_http);
            //************************** API V2 ***************************/

            $cek_tagihan = json_decode($cek_tagihan_http, true);
            $app->simpan_file("res_cek_pln.txt", $cek_tagihan_http);
            ...
            if (isset($_REQUEST['inq_id'], $_REQUEST['trx_id'], $_REQUEST['biaya_toko'])) {
                ...
                $app->simpan_file("bayar-$id_pelanggan.json", $bayar_tagihan_http);
                //**************************** API V2 ********************************/
...
<body class="bdy">
    <div style="background-color:<?= $primary ?>;">
        <div class="mx-auto py-5 mb-2 text-center">
            <img src="<?php echo $c_url; ?>/assets/img/tagihan_pln/listrik.png">
        </div>
    </div>
    <div class="container" style="margin-bottom:60px;">
        <div class="row ">
            <div class="py-3 mx-auto col-md-10">
                ...
                <div class="row mt-2">
                    <div class="col-10">
                        <input type="number" id="nope" class="form-control" placeholder="Masukkan Nomor Pelanggan" required>
                        <input type="hidden" id="csrf" class="form-control" value="<?php echo $app->csrf(); ?>">
                    </div>
                    <div class="col-auto">
                        <a id="inpo" onclick="custom()"><img class="mt-1 " src="<?php $c_url; ?>/assets/img/tagihan_pln/info.svg" width="30px"></a>
                    </div>
                </div>
                ...
                <div id="infome">
                    <div class="alert alert-success text-justify" style="margin-top:10px">
                        Bayar tagihan PLN di bukakios kamu otomatis dapet diskon biaya admin sebesar <b><?PHP echo $app->idr($profit); ?></b>
                    </div>
                    <div class="alert alert-warning text-justify" style="margin-top:10px">
                        Info.. Tagihan PLN pascabayar jatuh tempo pada tanggal 20 setiap bulannya. agar tidak terkena denda pastikan bayar sebelum tanggal tsb
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="card-body" style="padding: 0px;">
                    <div>
                        <table class="table">
                            ...
                            <tr>
                                <td width="55%%">Lembar Tagihan</td>
                                <td><span style="float:right;" id="lembar_tagihan"></span></td>
                            </tr>
                            <tr>
                            ...
                            <!--<tr>
                            <td width="55%%" >Profit Kamu</td>
                            <td><span style="float:right;" id="profit">Rp2.000</span></td>
                        </tr>-->
                            <tr>
        ...
        $(document).ready(function() {
                ...
                $.ajax({
                    ...
                    success: function(output) {
                        ...
                        if (myJsn.status == 1) {
                            ...
                            var lembar_tagihan = data.lembar_tagihan;
                            ...
                            $("#lembar_tagihan").html(lembar_tagihan + " Bulan");
                            $("#potongan").html("- " + rubah_rp(profit));
                            $("#denda").html(rubah_rp(denda));
                            $("#tarif_daya").html(tarif_daya);
            ...
            $('#pay').on('click', function() {
                if (inq_id != "") {
                    ...
                    $.ajax({
                        url: '<?= $file_me ?>?msg=bayar&biaya_toko=' + profit + '&id_pelanggan=' + id_pelanggan + '&csrf=' + csrf + '&trx_id=' + trx_id + '&inq_id=' + inq_id,
                        success: function(output) {
                            $('.footer').show();
                            $('#load').hide();
                            var myJsn = JSON.parse(output);
