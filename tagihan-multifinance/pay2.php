...
if (isset($_REQUEST['msg'], $_REQUEST['csrf'])) {
    ...
    if ($_SESSION['csrf'] === $csrf) {
        if ($msg === "cek") {
            ...
            // Debug log inquiry Multifinance
            if (isset($app) && method_exists($app, 'simpan_file')) {
                $app->simpan_file("inq_multi_" . preg_replace('/[^A-Za-z0-9]/', '', $id_pelanggan) . ".json", (string)$cek_tagihan_http);
            }
...
<body class="min-h-screen bg-slate-50 font-sans text-slate-950">
    ...
    <section class="px-4 pb-4">
      <div class="rounded-[16px] border border-slate-200 bg-white shadow-soft overflow-hidden">
        <div class="px-5 pt-5 pb-4" id="inputSection">
          ...
          <p class="text-[13px] text-mutedText mt-2">Masukkan nomor kontrak / ID tagihan <?= htmlspecialchars($product_name) ?></p>
        </div>
  ...
  <script>
    ...
    function showInfo() {
      showModal({
        title: 'Cek Nomor Kontrak',
        message: 'Nomor kontrak / ID tagihan tertera pada kartu kontrak, struk, atau invoice angsuran. Biasanya berupa angka/kode pelanggan. Jika kesulitan, hubungi perusahaan multifinance terkait.',
        variant: 'info',
        btnText: 'Ok'
      });
    }
    ...
    function renderDetail(data) {
      ...
      // Field khusus Multifinance/Pascabayar (fallback ke '-' jika kosong)
      ...
      // Sembunyikan baris meter_awal / meter_akhir kalau kosong (produk Multifinance biasanya tidak kirim)
      if (meterAwal === '-') {
