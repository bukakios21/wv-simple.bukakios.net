...
if (isset($_REQUEST['msg'], $_REQUEST['csrf'])) {
    ...
    if ($_SESSION['csrf'] === $csrf) {
        if ($msg === "cek") {
            ...
            $cek_tagihan_http = $api_v2->inq_pasca($kode_produk_tes, $id_pelanggan);
            ...
            if (isset($_REQUEST['trx_id'], $_REQUEST['biaya_toko'])) {
                ...
                // Debug log bayar Multifinance
                if (isset($app) && method_exists($app, 'simpan_file')) {
                    $app->simpan_file("pay_multi_" . preg_replace('/[^A-Za-z0-9]/', '', $trx_id) . ".json", (string)$bayar_tagihan_http);
                }
...
<head>
  ...
  <title><?= htmlspecialchars($product_name ?? 'Bayar Angsuran Multifinance') ?></title>
...
<body class="min-h-screen bg-slate-50 font-sans text-slate-950">
    ...
    <section class="px-4 pb-4">
      <div class="rounded-[16px] border border-slate-200 bg-white shadow-soft overflow-hidden">
        <div class="px-5 pt-5 pb-4" id="inputSection">
          <label class="text-[14px] font-semibold text-slate-700">Nomor Kontrak / ID Tagihan</label>
          ...
          <p class="text-[13px] text-mutedText mt-2">Masukkan nomor kontrak Multifinance <?= htmlspecialchars($product_name) ?></p>
        </div>
          ...
          <div class="rounded-xl bg-emerald-50 border border-emerald-100 px-4 py-3 flex items-start gap-3">
            ...
            <div class="flex-1 min-w-0">
              <p class="text-[13px] text-emerald-700 leading-snug">
                Bayar angsuran <?= htmlspecialchars($product_name) ?> di bukakios, kamu otomatis mendapatkan keuntungan dari biaya admin.
              </p>
            </div>
    ...
    <section id="detailSection" class="hidden px-4 pb-6">
      <div class="rounded-[20px] border border-slate-200 bg-white shadow-soft overflow-hidden">
        ...
        <div class="divide-y divide-slate-100">
          ...
          <div class="flex items-center justify-between px-5 py-3">
            <span class="text-[14px] text-slate-600">Jatuh Tempo</span>
            <span class="text-[14px] font-semibold text-slate-800" id="jatuh_tempo">-</span>
          </div>
          <div class="flex items-center justify-between px-5 py-3">
  ...
  <script>
    ...
    function showInfo() {
      showModal({
        title: 'Cek Nomor Kontrak Multifinance',
        message: 'Nomor kontrak Multifinance tertera pada kartu kontrak, struk, atau invoice bulanan dari perusahaan finance terkait. Biasanya berupa angka 8–18 digit. Jika kesulitan, hubungi perusahaan finance terdekat.',
        variant: 'info',
        btnText: 'Ok'
      });
    }
    ...
    function renderDetail(data) {
      // API bisa return nested (data.data / custom_data) atau flat (langsung field)
      ...
      // Field khusus Multifinance (fallback ke '-' jika kosong)
      var namaPelanggan = d.customer_name || d.nama_pelanggan || '-';
      var jatuhTempo    = d.jatuh_tempo || d.tanggal_jatuh_tempo || '-';
      ...
      document.getElementById('jatuh_tempo').textContent    = jatuhTempo;
