...
if (isset($_GET['wv3'])) {
    if ($_GET['wv3'] == "1") {
        $id_produk = $_GET['id'];
        $nama_produk = $_GET['name'];
        header("Location: https://wv3.bukakios.id/angsuran-kredit/$id_produk?name=$nama_produk");
        exit();
    }
}
...
if (isset($_REQUEST['msg'], $_REQUEST['csrf'])) {
    ...
    if ($_SESSION['csrf'] === $csrf) {
        if ($msg === "cek") {
            ...
            $app->simpan_file("cek-$kode_produk.json", $cek_tagihan_http);
            //************************** API V2 ***************************/
            ...
            if (isset($_REQUEST['inq_id'], $_REQUEST['trx_id'], $_REQUEST['biaya_toko'])) {
                ...
                $app->simpan_file("bayar-$kode_produk.json", $bayar_tagihan_http);
                //**************************** API V2 ********************************/
...
if (isset($data_produk['status']) and $data_produk['status'] == 1) {
    ...
    $html_title = "Gagal";
    $lyt_button_link = "$c_url/tagihan-multifinance/";
...
<head>
    ...
    <title>title::Angsuran Kredit</title>
    <style>
        html * {
                ...
                <div class="card-body" style="padding: 0px;">
                    <div>
                        <table class="table">
                            ...
                            <tr>
                                <td width="55%%" height="10px">Jatuh Tempo</td>
                                <td height="10px"><span style="float:right;" id="jatuh_tempo"></span></td>
                            </tr>
                            <tr>
